...
$$\frac{1}{e^{\lambda t}}, \frac{1}{2.72^{\lambda t}}, \frac{1}{`r round(2.72^1,1)`}, \frac{1}{`r round(2.72^2,2)`}, \frac{1}{`r round(2.72^3,2)`}, \frac{1}{`r round(2.72^4,2)`}$$

$$\frac{1}{e^{\lambda t}}, \frac{1}{2.72^{\lambda t}}, `r round(1/ 2.72^1,1)`, `r round(1/ 2.72^2,2)`, `r round(1/2.72^3,2)`, `r round(1/2.72^4,2)`$$ 
...
```{r ratesrisk} 
rr <- tibble(lt = seq(0.001, 2, 0.001))
rr <- rr %>% 
  mutate(risk = 1 - exp(-lt))
lbl <- bquote("\U03BB\U0074")
rr_plot <- ggplot(rr, aes(x = lt, y = risk)) +
  geom_line() +
  scale_y_continuous("Risk (proportion)") +
  scale_x_continuous(paste0("Rate x time (", lbl, ")"))
rr_plot
```
...
```{r example_constant}
rate_con <- expand.grid(rate = c(0.1, 0.2, 0.3, 0.4), time = c(1, 2, 3, 4)) %>% 
  as_tibble()
rate_con <- rate_con %>% 
  mutate(ci = round(1 - exp(-rate * time),2),
         time = paste(time, "year(s)")) %>% 
  spread(time, ci)
knitr::kable(rate_con)
```
...
6. Why are the answers to question 1 and question 2 the same?
*Because $\lambda \times t$ is the same for both.*

7. Why is it that for the first three we would have gotten a similar answer if we just multiplied the rate per year by the number of years, but this is not the case for the last two questions?
*For the first three, the rate and follow-up time are short (ie $\lambda \times t$ is small) whereas in the latter two questions it is large*

8. Why is the answer to question 2 similar to, but not exactly, ten times the answer to question 2?
*Because $\lambda \times t$ is quite low for both examples and so the relationship between $\lambda \times t$ and risk is close to linear in this range, but is not quite linear.*

9. What assumptions do all five questions make?
*There are no competing risks. The rates are constant over time*
...
## Time-varying rates
  ...
  $$ Risk = 1 - e^{-\int(\lambda_t dt)}$$
  - Rates constant within discrete time periods
...
```{r area_curve}
ac <- tibble(time = 0:10,
             changing = 10 + rnorm(11),
             constant = 10) %>% 
  gather("for_facet", "Rate", -time)

ac_dis <- tibble(time = c(0, 5, 5.001, 10),
                 Rate = c(9, 9, 11, 11)) %>% 
  mutate(for_facet = "discrete")

ac <- bind_rows(ac, ac_dis) %>% 
  mutate(Rate = Rate/10,
         for_facet = factor(for_facet, levels = c("constant", "changing", "discrete")))

ac_lbl <- tibble(time = rep(5, 3), Rate = rep(0.5,3),
                 for_facet = c("changing", "constant", "discrete"),
                 label = c(bquote("\U222B\U03BB\U0074"),
                           bquote("\U03BB\U0074"),
                           bquote("\U2211\U03BB\U0074"))) %>% 
  mutate(for_facet = factor(for_facet, levels = c("constant", "changing", "discrete")))


ac_plot <- ggplot(ac, aes(x = time, y = Rate, ymin = 0L)) +
  geom_area(alpha = 0.5, fill = "blue") +
  facet_grid(~for_facet) +
  geom_label(data = ac_lbl, mapping = aes(label = label))
ac_plot
...
# print(a)
set.seed(a)
rate_error <- rnorm(365*5, 0, 0.01)
rate <- 50 * cumprod(exp((rate_error)))
rate_df <- tibble(time = 1:(5*365), rate = rate)
rate_df <- rate_df %>% 
  mutate(cumulative_incidence = 1 - exp(-cumsum(rate/100 * 1/365.25)))

plot1 <- ggplot(rate_df, aes(x = time/365.25, y = rate)) + 
  geom_line() + 
  scale_y_continuous("Incidence rate \n per 100 py") +
  scale_x_continuous("Time (years)")

plot2 <- ggplot(rate_df, aes(x = time/365.25, y = cumulative_incidence)) +
  geom_line() + 
...
```{r ci_table_plot}
rate_df_agg <- rate_df %>%
  mutate(half_year = 1 + trunc(time/(365.25/2))) %>% 
  group_by(half_year) %>% 
  summarise(event = max(cumulative_incidence) - min(cumulative_incidence)) %>% 
  ungroup() %>% 
  mutate(event = round(1000 * event))

rate_df_agg <- rate_df_agg %>% 
  mutate(at_risk_at_end = 1000 - cumsum(event),
         pt = (0.5*lag(at_risk_at_end, default = 1000)) - (0.25*event),
         rate = 100*event/pt,
         time = half_year * 365.25/2)
plot1_bars <-  plot1 + 
  geom_point(data = rate_df_agg, mapping = aes(x = (time/365.25)-0.25, y = rate), 
             colour = "red", size = 15, shape = 3)
plot1_bars
# 
```
...
```{r ci_table}
rate_df_agg2 <- rate_df_agg %>% 
  mutate(N_at_risk = at_risk_at_end + event,
         duration = 0.5) %>% 
  select(time_period = half_year,
         N_at_risk,
        duration,
         events = event,
         person_time = pt,
         rate)
knitr::kable(rate_df_agg2, digits = 1)
```

## Table for incidence rate at different time periods, continued

```{r ci_table_calc}
rate_df_agg3 <-  rate_df_agg2 %>% 
  mutate(Risk = paste0(100*cumsum(events)/1000, "%"),
         duration = 0.5)
knitr::kable(rate_df_agg3, digits = 1)
```
...
Set "Rate of target event per 100 person-years:" to 10, and the "Rate ratio for effect of treatment on target event:" to 0.78. Leave the other settings as they are. Examine the effect of increasing the "Rate of target event per 100 person-years:". What impact do these changes have on the:-
 ...
 1. Risk of target events
 *It increases, but by less with each increase.* 
 2. Relative risk
 *It gets closer to one*
 3. Absolute risk reduction
 *Over the initial year it gets larger, but it is smaller over longer time periods.*
 
 4. What happens to the relationship between the rate ratio and risk ratio as you increase the target event rate?
*It tends towards the null*

 5. What implications does this have for interpreting rate ratios?
*Long follow-up times or large rates mean that rate ratios ratios cannot be interpreted as risk ratios*

# Censoring

```{r elephant, fig.width=5}
knitr::include_graphics("Figures/Elephant.png")
...
plot_cens <- 
  ...
  3,1,96,
  3,7,1)
plot_cens_plit <- ggplot(plot_cens, aes(x = x, y = as.integer(y), shape = as.integer(censor), group = y)) +
  geom_point(size = 5) +
  geom_line() +
  scale_shape_identity() +
  scale_x_continuous("Time") +
  scale_y_continuous("Patient", breaks = 1:3)
plot_cens_plit
...
```{r noncensorcompete}
nocens <- tibble(Relapse = rpois(8, 20),
                 Death = rpois(8, 10),
                 Either = Relapse + Death)
nocens <- nocens %>% 
  mutate_all(list(Cum = cumsum, Risk = function(x) cumsum(x)/1000))
knitr::kable(nocens)
```
...
```{r km_niave}
knitr::include_graphics("Figures/km_niave.png")

```

## Why

```{r km_niave2, fig.align="left", fig.width = 3}
knitr::include_graphics("Figures/km_niave.png")
...
$$ S(t) = exp^{- \int_0^t{\lambda_1(t) + \lambda_2(t) dt}} $$
 ...
 - At any time, survival plus Risk<sub>1</sub> plus Risk<sub>2</sub> always equals 1.
 
 - Work through an example in excel
 
## Area under the curve

```{r area_curve_compete}
ac <- tibble(time = 0:10,
             non_comp = 10 + rnorm(11),
             comp1 = 5 + rnorm(11),
             comp22 = non_comp - comp1) %>% 
  gather("rate_grp", "Rate", -time)

ac <- ac %>% 
  mutate(Rate = Rate/10,
         for_facet = if_else(!rate_grp == "non_comp", "B:Competing", "A:Non-competing"))

ac_lbl <- tibble(time = rep(5, 2), Rate = rep(0.5,2),
                 for_facet = c("A:Non-competing", "B:Competing"),
                 label = c(bquote("\U222B\U03BB\U0074"),
                           bquote("\U222B\U03BB\U0074\U0031\U03BB\U0074\U0032")),
                 rate_grp = "")


ac_plot <- ggplot(ac, aes(x = time, y = Rate, fill = rate_grp, ymin = 0L)) +
  geom_area(alpha = 0.5) +
  facet_grid(~for_facet) +
  geom_label(data = ac_lbl, mapping = aes(label = label), fill = "white") +
...
```{r km_niave_other}
knitr::include_graphics("Figures/km_niave_ci.png")
...
You will also find the link to this on the front-page of https://dmcalli2.github.io/Advanced_epidemiology_course/
